...
-- 017_repair_unresolved_identity_candidates.sql
-- Server-side repair pass for staged identity rows that are still UNRESOLVED.
-- Staging only: does not write canonical business tables.
...
begin
    for r in
        ...
        where (p_ingestion_job_id is null or ingestion_job_id = p_ingestion_job_id)
          ...
          and coalesce(resolution_status,'UNRESOLVED') in ('UNRESOLVED','INVALID','NEW','MATCHED','AMBIGUOUS')
    ...
    loop
        ...
        where active and table_name = r.target_table
        limit 1;
            ...
            update pc_staged_records
               set resolution_status='INVALID',
                   ...
                   resolution_details=coalesce(resolution_details,'{}'::jsonb)
                       || jsonb_build_object('repair_sql','017','reason','NO_ENTITY_METADATA')
        ...
        -- First try canonical primary key if the payload already contains one.
            ...
            if v_count = 1 then
                update pc_staged_records
                   set resolution_status='MATCHED',
                       ...
                       candidate_count=1,
                       resolution_details=coalesce(resolution_details,'{}'::jsonb)
                           || jsonb_build_object('repair_sql','017','candidate_kind','MATCHED')
                 where staged_record_id=r.staged_record_id;
        ...
        -- Then use the configured canonical display field as an exact normalized identity check.
        v_name := case
            when e.display_name_column is not null then nullif(trim(v_payload ->> e.display_name_column),'')
            else null
        end;

        if v_name is not null and e.display_name_column is not null then
        ...
        if v_count = 1 then
            ...
            update pc_staged_records
               set resolution_status='AMBIGUOUS',
                   ...
                   resolution_details=coalesce(resolution_details,'{}'::jsonb)
                       || jsonb_build_object('repair_sql','017','candidate_kind','AMBIGUOUS')
             where staged_record_id=r.staged_record_id;
            v_ambiguous := v_ambiguous + 1;

        else
            -- No canonical match: this is genuinely NEW. Generate/reuse a deterministic candidate ID.
            ...
            update pc_staged_records
               set payload=v_payload,
                   ...
                   validation_status=case when validation_status='invalid' then 'pending' else validation_status end,
                   resolution_details=coalesce(resolution_details,'{}'::jsonb)
                       || jsonb_build_object('repair_sql','017','candidate_kind','NEW','candidate_id',v_id)
             where staged_record_id=r.staged_record_id;
            perform pc_expand_staged_payload(r.staged_record_id);
            v_new := v_new + 1;
        end if;
    ...
    if p_ingestion_job_id is not null then
        update pc_ingestion_jobs
           set stats=coalesce(stats,'{}'::jsonb) || jsonb_build_object(
               'repair017_total',v_total,
               'repair017_matched',v_matched,
               'repair017_new',v_new,
               'repair017_ambiguous',v_ambiguous,
               'repair017_invalid',v_invalid
           )
...
'Repairs unresolved staged identities using canonical primary key/name checks, classifies no-match rows as NEW, and assigns deterministic candidate IDs in staging only.';
