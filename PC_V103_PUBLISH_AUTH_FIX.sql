-- P&C v1.0.3: fix legacy JWT-role checks blocking backup and publish RPCs.
-- Run as database owner in Supabase SQL Editor after 03_SUPABASE_V10_PUBLISH.sql.
-- Existing tables, staged records, approvals and immutable backups are unchanged.
-- SECURITY DEFINER functions remain EXECUTE-restricted to service_role and postgres.
-- This fixes two functions, not the separate v0.7 queue claim.
...
begin
 -- Execution is restricted to service_role by explicit function grants below.
...
-- Explicit least-privilege execution gates. Do not grant anon/authenticated.
REVOKE ALL ON FUNCTION public.pc_v10_backup_staged(uuid,uuid[],text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.pc_v10_publish_approved(uuid,uuid[],uuid,text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.pc_v10_backup_staged(uuid,uuid[],text) TO service_role, postgres;
GRANT EXECUTE ON FUNCTION public.pc_v10_publish_approved(uuid,uuid[],uuid,text) TO service_role, postgres;
