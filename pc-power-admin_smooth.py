        ...
        with tabs[1]:
            if not jid:
                ...
                if any(str(r.get("resolution_status") or "").upper()=="BROKEN_REFERENCE" for r in blocked_rows):
                    st.info(
                        "BROKEN_REFERENCE can also mean the child row still carries the workbook/logical ID "
                        "while the parent was matched to a different canonical ID. Migration 039 remaps those "
                        "same-job endpoints automatically before retrying event links/relationships."
                    )
