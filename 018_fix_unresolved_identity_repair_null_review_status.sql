...
-- 018_fix_unresolved_identity_repair_null_review_status.sql
-- Fixes SQL 017 so staged rows with NULL review_status are treated as pending.
            ...
            update pc_staged_records
               set resolution_status='INVALID',
                   ...
                   resolution_details=coalesce(resolution_details,'{}'::jsonb)
                       || jsonb_build_object('repair_sql','018','reason','NO_ENTITY_METADATA')
            ...
            if v_count = 1 then
                update pc_staged_records
                   set resolution_status='MATCHED',
                       ...
                       candidate_count=1,
                       resolution_details=coalesce(resolution_details,'{}'::jsonb)
                           || jsonb_build_object('repair_sql','018','candidate_kind','MATCHED')
                 where staged_record_id=r.staged_record_id;
        ...
        if v_count = 1 then
            ...
            update pc_staged_records
               set resolution_status='AMBIGUOUS',
                   ...
                   resolution_details=coalesce(resolution_details,'{}'::jsonb)
                       || jsonb_build_object('repair_sql','018','candidate_kind','AMBIGUOUS')
             where staged_record_id=r.staged_record_id;
            ...
            if v_id is null then
                v_id := v_prefix || '_' ||
                        upper(substr(encode(digest(v_seed,'sha256'),'hex'),1,16));
            end if;
    ...
    if p_ingestion_job_id is not null then
        update pc_ingestion_jobs
           set stats=coalesce(stats,'{}'::jsonb) || jsonb_build_object(
               'repair018_total',v_total,
               'repair018_matched',v_matched,
               'repair018_new',v_new,
               'repair018_ambiguous',v_ambiguous,
               'repair018_invalid',v_invalid
           )
...
'SQL 018: repairs unresolved staged identities including rows with NULL review_status; matches canonical IDs/names or classifies as NEW with deterministic staging IDs.';
