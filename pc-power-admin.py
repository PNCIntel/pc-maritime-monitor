...
def _schema_valid(table,payload):
    if table in PHASE13_TABLES:
        return _phase13_schema_valid(table,payload)
...
def _fk_valid(sb,table,payload):
    if table in PHASE13_TABLES:
        return _phase13_fk_valid(sb,table,payload)
    problems=[]
...
def _fill_staging_key(payload,target_table,natural_key):
    ...
    if target_table in PHASE13_TABLES and not natural_key:
        return payload
...
# ===============================================================
# PHASE 13G-2 — STREAMLIT MULTIMODAL INTEGRATION (2026-09-23)
# Generated from the six supplied Phase 13 SQL migrations.
# Offline-ready. New domain records are STAGING-ONLY by default.
# Documents, sanctions and all existing Streamlit pages are retained.
# ===============================================================
...
PHASE13_TABLES = frozenset(t for group in PHASE13_TABLES_BY_PHASE.values() for t in group)
...
PHASE13_COLUMNS = {'pc_air_service_movements': ['air_service_movement_id',
 ...
 'pc_weather_observations': ['weather_observation_id',
                             ...
                             'metadata',
                             'created_at']}
PHASE13_REQUIRED = {'pc_air_service_movements': [],
 'pc_airspace_disruptions': ['event_id'],
 'pc_canonical_identity_decisions': ['source_entity_id', 'match_status'],
 'pc_claim_evidence': ['research_claim_id', 'evidence_role'],
 'pc_company_asset_roles': ['entity_id', 'asset_role'],
 'pc_company_corridor_roles': ['entity_id', 'corridor_key', 'corridor_role'],
 'pc_company_event_effects': ['entity_id', 'event_id', 'effect_type'],
 'pc_company_milestones': ['entity_id', 'milestone_type', 'title'],
 'pc_company_offices': ['entity_id', 'office_type'],
 'pc_company_people_roles': ['entity_id', 'person_id', 'position_title'],
 'pc_company_portfolio_positions': ['holder_entity_id', 'position_type'],
 'pc_corridor_exposures': ['corridor_key', 'exposure_type'],
 'pc_corridor_mode_connections': ['corridor_key', 'from_asset_id', 'to_asset_id', 'mode'],
 'pc_corridor_variants': ['corridor_key', 'variant_name', 'variant_type'],
 'pc_cruise_voyage_calls': ['cruise_voyage_id', 'port_asset_id', 'sequence_no'],
 'pc_cruise_voyages': ['mobile_asset_id'],
 'pc_defence_organisations': ['organisation_type'],
 'pc_defence_programme_milestones': ['defence_programme_id', 'milestone_type'],
 'pc_defence_programme_participants': ['defence_programme_id', 'entity_id', 'participant_role'],
 'pc_defence_programmes': ['programme_name', 'programme_type'],
 'pc_event_updates': ['event_id', 'update_type', 'summary'],
 'pc_freight_rate_assessments': ['mode', 'rate_category', 'observed_date', 'assessment_kind'],
 'pc_inland_waterway_status': ['observed_at'],
 'pc_logistics_event_impacts': ['event_id', 'impact_type'],
 'pc_logistics_service_agreements': ['service_type'],
 'pc_logistics_shipment_legs': ['logistics_shipment_id', 'leg_no', 'mode'],
 'pc_logistics_shipments': [],
 'pc_market_corridor_links': ['market_observation_id', 'corridor_key', 'relevance_type'],
 'pc_monitoring_alerts': ['monitoring_profile_id', 'headline'],
 'pc_monitoring_observations': ['monitoring_profile_id', 'observed_at', 'observation_type'],
 'pc_monitoring_profiles': ['profile_name', 'profile_domain'],
 'pc_monitoring_rules': ['monitoring_profile_id', 'rule_name', 'rule_type'],
 'pc_passenger_incident_impacts': ['event_id', 'impact_type'],
 'pc_people': ['display_name'],
 'pc_port_service_disruptions': ['event_id', 'port_asset_id', 'disruption_type'],
 'pc_rail_network_disruptions': ['event_id', 'disruption_type'],
 'pc_rail_rolling_stock_units': ['unit_identifier', 'rolling_stock_type'],
 'pc_rail_service_runs': [],
 'pc_research_claims': ['claim_text'],
 'pc_research_hypotheses': ['research_project_id', 'hypothesis_text'],
 'pc_research_opportunities': ['research_project_id', 'opportunity_type', 'title'],
 'pc_research_projects': ['title', 'research_domain'],
 'pc_research_questions': ['research_project_id', 'question_text'],
 'pc_road_border_observations': ['border_asset_id', 'observed_at'],
 'pc_road_disruptions': ['event_id', 'disruption_type'],
 'pc_road_network_links': [],
 'pc_screening_cases': ['subject_type', 'submitted_name', 'screening_purpose'],
 'pc_screening_matches': ['screening_case_id', 'sanctions_designation_id'],
 'pc_security_attribution_claims': ['event_id', 'attribution_kind', 'claim_summary'],
 'pc_security_operation_participants': ['security_operation_id', 'participant_role'],
 'pc_security_operations': ['operation_name', 'operation_type'],
 'pc_shipbuilding_production_tasks': ['shipyard_asset_id', 'task_type'],
 'pc_shipyard_capacity_history': ['shipyard_asset_id', 'observed_date', 'metric_name'],
 'pc_supply_chain_dependencies': ['dependency_type'],
 'pc_trucking_fleet_assignments': ['entity_id', 'mobile_asset_id', 'relationship_type'],
 'pc_vessel_deployments': ['mobile_asset_id', 'deployment_type'],
 'pc_vessel_identity_history': ['mobile_asset_id', 'identifier_type', 'identifier_value'],
 'pc_weather_observations': ['provider', 'weather_kind', 'hazard_type']}
PHASE13_FK = {'pc_air_service_movements': [('transport_service_id', 'pc_transport_services', 'transport_service_id'),
                              ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                              ('operator_entity_id', 'pc_entities', 'entity_id'),
                              ('departure_airport_asset_id', 'pc_assets', 'asset_id'),
                              ('arrival_airport_asset_id', 'pc_assets', 'asset_id'),
                              ('related_event_id', 'pc_events', 'event_id'),
                              ('source_id', 'pc_sources', 'source_id'),
                              ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_airspace_disruptions': [('event_id', 'pc_events', 'event_id'),
                             ('issuing_authority_entity_id', 'pc_entities', 'entity_id'),
                             ('affected_airport_asset_id', 'pc_assets', 'asset_id'),
                             ('source_id', 'pc_sources', 'source_id'),
                             ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_canonical_identity_decisions': [('source_entity_id', 'pc_entities', 'entity_id'),
                                     ('canonical_entity_id', 'pc_entities', 'entity_id'),
                                     ('source_id', 'pc_sources', 'source_id'),
                                     ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_claim_evidence': [('research_claim_id', 'pc_research_claims', 'research_claim_id'),
                       ('source_id', 'pc_sources', 'source_id'),
                       ('source_record_uuid', 'pc_source_records', 'source_record_uuid'),
                       ('document_id', 'pc_documents', 'document_id')],
 'pc_company_asset_roles': [('entity_id', 'pc_entities', 'entity_id'),
                            ('asset_id', 'pc_assets', 'asset_id'),
                            ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                            ('source_id', 'pc_sources', 'source_id'),
                            ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_company_corridor_roles': [('entity_id', 'pc_entities', 'entity_id'),
                               ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                               ('transport_service_id', 'pc_transport_services', 'transport_service_id'),
                               ('source_id', 'pc_sources', 'source_id'),
                               ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_company_event_effects': [('entity_id', 'pc_entities', 'entity_id'),
                              ('event_id', 'pc_events', 'event_id'),
                              ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                              ('source_id', 'pc_sources', 'source_id'),
                              ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_company_milestones': [('entity_id', 'pc_entities', 'entity_id'),
                           ('event_id', 'pc_events', 'event_id'),
                           ('transaction_id', 'pc_transactions', 'transaction_id'),
                           ('related_entity_id', 'pc_entities', 'entity_id'),
                           ('related_asset_id', 'pc_assets', 'asset_id'),
                           ('source_id', 'pc_sources', 'source_id'),
                           ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_company_offices': [('entity_id', 'pc_entities', 'entity_id'),
                        ('site_asset_id', 'pc_assets', 'asset_id'),
                        ('related_agent_entity_id', 'pc_entities', 'entity_id'),
                        ('source_id', 'pc_sources', 'source_id')],
 'pc_company_people_roles': [('entity_id', 'pc_entities', 'entity_id'),
                             ('person_id', 'pc_people', 'person_id'),
                             ('office_id', 'pc_company_offices', 'company_office_id'),
                             ('source_id', 'pc_sources', 'source_id'),
                             ('document_id', 'pc_documents', 'document_id')],
 'pc_company_portfolio_positions': [('holder_entity_id', 'pc_entities', 'entity_id'),
                                    ('investee_entity_id', 'pc_entities', 'entity_id'),
                                    ('investee_asset_id', 'pc_assets', 'asset_id'),
                                    ('investment_vehicle_entity_id', 'pc_entities', 'entity_id'),
                                    ('linked_transaction_id', 'pc_transactions', 'transaction_id'),
                                    ('source_id', 'pc_sources', 'source_id'),
                                    ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_corridor_exposures': [('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                           ('corridor_variant_id', 'pc_corridor_variants', 'corridor_variant_id'),
                           ('affected_segment_key', 'pc_corridor_segments', 'segment_key'),
                           ('event_id', 'pc_events', 'event_id'),
                           ('source_id', 'pc_sources', 'source_id'),
                           ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_corridor_mode_connections': [('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                                  ('from_asset_id', 'pc_assets', 'asset_id'),
                                  ('to_asset_id', 'pc_assets', 'asset_id'),
                                  ('rail_link_id', 'pc_rail_links', 'rail_link_id'),
                                  ('road_corridor_id', 'pc_road_corridors', 'road_corridor_id'),
                                  ('transport_service_id',
                                   'pc_transport_services',
                                   'transport_service_id'),
                                  ('source_id', 'pc_sources', 'source_id'),
                                  ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_corridor_variants': [('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                          ('source_id', 'pc_sources', 'source_id')],
 'pc_cruise_voyage_calls': [('cruise_voyage_id', 'pc_cruise_voyages', 'cruise_voyage_id'),
                            ('port_asset_id', 'pc_assets', 'asset_id'),
                            ('related_event_id', 'pc_events', 'event_id'),
                            ('original_port_asset_id', 'pc_assets', 'asset_id'),
                            ('source_id', 'pc_sources', 'source_id'),
                            ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_cruise_voyages': [('cruise_itinerary_id', 'pc_cruise_itineraries', 'cruise_itinerary_id'),
                       ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                       ('cruise_line_entity_id', 'pc_entities', 'entity_id'),
                       ('homeport_asset_id', 'pc_assets', 'asset_id'),
                       ('source_id', 'pc_sources', 'source_id'),
                       ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_defence_organisations': [('entity_id', 'pc_entities', 'entity_id'),
                              ('parent_entity_id', 'pc_entities', 'entity_id'),
                              ('source_id', 'pc_sources', 'source_id'),
                              ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_defence_programme_milestones': [('defence_programme_id',
                                      'pc_defence_programmes',
                                      'defence_programme_id'),
                                     ('shipbuilding_order_unit_id',
                                      'pc_shipbuilding_order_units',
                                     ...
                                     ('source_id', 'pc_sources', 'source_id'),
                                     ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_defence_programme_participants': [('defence_programme_id',
                                        'pc_defence_programmes',
                                        'defence_programme_id'),
                                       ('entity_id', 'pc_entities', 'entity_id'),
                                       ('shipyard_asset_id', 'pc_assets', 'asset_id'),
                                       ('source_id', 'pc_sources', 'source_id'),
                                       ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_defence_programmes': [('customer_entity_id', 'pc_entities', 'entity_id'),
                           ('lead_contractor_entity_id', 'pc_entities', 'entity_id'),
                           ('shipbuilding_order_id', 'pc_shipbuilding_orders', 'shipbuilding_order_id'),
                           ('contract_id', 'pc_contracts', 'contract_id'),
                           ('source_id', 'pc_sources', 'source_id'),
                           ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_event_updates': [('event_id', 'pc_events', 'event_id'),
                      ('source_id', 'pc_sources', 'source_id'),
                      ('source_record_uuid', 'pc_source_records', 'source_record_uuid'),
                      ('document_id', 'pc_documents', 'document_id')],
 'pc_freight_rate_assessments': [('market_observation_id',
                                 ...
                                 ('source_id', 'pc_sources', 'source_id'),
                                 ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_inland_waterway_status': [('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                               ('waterway_asset_id', 'pc_assets', 'asset_id'),
                               ('related_event_id', 'pc_events', 'event_id'),
                               ('source_id', 'pc_sources', 'source_id'),
                               ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_logistics_event_impacts': [('event_id', 'pc_events', 'event_id'),
                                ('logistics_shipment_id',
                                 'pc_logistics_shipments',
                                 'logistics_shipment_id'),
                                ('shipment_leg_id', 'pc_logistics_shipment_legs', 'shipment_leg_id'),
                                ('transport_service_id',
                                 'pc_transport_services',
                                 'transport_service_id'),
                                ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                                ('source_id', 'pc_sources', 'source_id'),
                                ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_logistics_service_agreements': [('contract_id', 'pc_contracts', 'contract_id'),
                                     ('customer_entity_id', 'pc_entities', 'entity_id'),
                                     ('provider_entity_id', 'pc_entities', 'entity_id'),
                                     ('agent_entity_id', 'pc_entities', 'entity_id'),
                                     ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                                     ('transport_service_id',
                                      'pc_transport_services',
                                      'transport_service_id'),
                                     ('source_id', 'pc_sources', 'source_id'),
                                     ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_logistics_shipment_legs': [('logistics_shipment_id',
                                 'pc_logistics_shipments',
                                 'logistics_shipment_id'),
                                ('transport_service_id',
                                 'pc_transport_services',
                                ...
                                ('source_id', 'pc_sources', 'source_id'),
                                ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_logistics_shipments': [('origin_asset_id', 'pc_assets', 'asset_id'),
                            ('destination_asset_id', 'pc_assets', 'asset_id'),
                            ('shipper_entity_id', 'pc_entities', 'entity_id'),
                            ('consignee_entity_id', 'pc_entities', 'entity_id'),
                            ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                            ('source_id', 'pc_sources', 'source_id'),
                            ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_market_corridor_links': [('market_observation_id',
                               'pc_market_observations',
                               'market_observation_id'),
                              ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                              ('corridor_variant_id', 'pc_corridor_variants', 'corridor_variant_id'),
                              ('source_id', 'pc_sources', 'source_id'),
                              ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_monitoring_alerts': [('monitoring_profile_id', 'pc_monitoring_profiles', 'monitoring_profile_id'),
                          ('monitoring_rule_id', 'pc_monitoring_rules', 'monitoring_rule_id'),
                          ('monitoring_observation_id',
                           'pc_monitoring_observations',
                           'monitoring_observation_id'),
                          ('event_id', 'pc_events', 'event_id')],
 'pc_monitoring_observations': [('monitoring_profile_id',
                                 'pc_monitoring_profiles',
                                 'monitoring_profile_id'),
                                ('monitoring_rule_id', 'pc_monitoring_rules', 'monitoring_rule_id'),
                                ('event_id', 'pc_events', 'event_id'),
                                ...
                                ('source_record_uuid', 'pc_source_records', 'source_record_uuid'),
                                ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_monitoring_profiles': [('research_project_id', 'pc_research_projects', 'research_project_id')],
 'pc_monitoring_rules': [('monitoring_profile_id', 'pc_monitoring_profiles', 'monitoring_profile_id')],
 'pc_passenger_incident_impacts': [('event_id', 'pc_events', 'event_id'),
                                   ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                                   ('cruise_voyage_id', 'pc_cruise_voyages', 'cruise_voyage_id'),
                                   ('ferry_route_id', 'pc_ferry_routes', 'ferry_route_id'),
                                   ('source_id', 'pc_sources', 'source_id'),
                                   ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_people': [('source_id', 'pc_sources', 'source_id')],
 'pc_port_service_disruptions': [('event_id', 'pc_events', 'event_id'),
                                 ('port_asset_id', 'pc_assets', 'asset_id'),
                                 ('terminal_asset_id', 'pc_assets', 'asset_id'),
                                 ('transport_service_id',
                                  'pc_transport_services',
                                 ...
                                 ('source_id', 'pc_sources', 'source_id'),
                                 ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_rail_network_disruptions': [('event_id', 'pc_events', 'event_id'),
                                 ('rail_network_id', 'pc_rail_networks', 'rail_network_id'),
                                 ('rail_link_id', 'pc_rail_links', 'rail_link_id'),
                                 ('node_asset_id', 'pc_assets', 'asset_id'),
                                 ('source_id', 'pc_sources', 'source_id'),
                                 ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_rail_rolling_stock_units': [('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                                 ('rail_network_id', 'pc_rail_networks', 'rail_network_id'),
                                 ('owner_entity_id', 'pc_entities', 'entity_id'),
                                 ('operator_entity_id', 'pc_entities', 'entity_id'),
                                 ('source_id', 'pc_sources', 'source_id'),
                                 ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_rail_service_runs': [('transport_service_id', 'pc_transport_services', 'transport_service_id'),
                          ('rail_network_id', 'pc_rail_networks', 'rail_network_id'),
                          ('operator_entity_id', 'pc_entities', 'entity_id'),
                          ('origin_asset_id', 'pc_assets', 'asset_id'),
                          ('destination_asset_id', 'pc_assets', 'asset_id'),
                          ('linked_event_id', 'pc_events', 'event_id'),
                          ('source_id', 'pc_sources', 'source_id'),
                          ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_research_claims': [('research_project_id', 'pc_research_projects', 'research_project_id'),
                        ('research_question_id', 'pc_research_questions', 'research_question_id'),
                        ('supersedes_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_research_hypotheses': [('research_project_id', 'pc_research_projects', 'research_project_id'),
                            ('alternative_to_id', 'pc_research_hypotheses', 'research_hypothesis_id')],
 'pc_research_opportunities': [('research_project_id', 'pc_research_projects', 'research_project_id'),
                               ('sponsor_entity_id', 'pc_entities', 'entity_id'),
                               ('candidate_entity_id', 'pc_entities', 'entity_id'),
                               ('corridor_key', 'pc_trade_corridors', 'corridor_key')],
 'pc_research_projects': [],
 'pc_research_questions': [('research_project_id', 'pc_research_projects', 'research_project_id'),
                           ('parent_question_id', 'pc_research_questions', 'research_question_id')],
 'pc_road_border_observations': [('border_asset_id', 'pc_assets', 'asset_id'),
                                 ('road_corridor_id', 'pc_road_corridors', 'road_corridor_id'),
                                 ('related_event_id', 'pc_events', 'event_id'),
                                 ('source_id', 'pc_sources', 'source_id'),
                                 ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_road_disruptions': [('event_id', 'pc_events', 'event_id'),
                         ('road_corridor_id', 'pc_road_corridors', 'road_corridor_id'),
                         ('road_network_link_id', 'pc_road_network_links', 'road_network_link_id'),
                         ('border_asset_id', 'pc_assets', 'asset_id'),
                         ('source_id', 'pc_sources', 'source_id'),
                         ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_road_network_links': [('road_corridor_id', 'pc_road_corridors', 'road_corridor_id'),
                           ('from_asset_id', 'pc_assets', 'asset_id'),
                           ('to_asset_id', 'pc_assets', 'asset_id'),
                           ('source_id', 'pc_sources', 'source_id'),
                           ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_screening_cases': [],
 'pc_screening_matches': [('screening_case_id', 'pc_screening_cases', 'screening_case_id'),
                          ('sanctions_designation_id',
                           'pc_sanctions_designations',
                           'sanctions_designation_id')],
 'pc_security_attribution_claims': [('event_id', 'pc_events', 'event_id'),
                                    ('claimed_actor_entity_id', 'pc_entities', 'entity_id'),
                                    ('source_id', 'pc_sources', 'source_id'),
                                    ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_security_operation_participants': [('security_operation_id',
                                         'pc_security_operations',
                                         'security_operation_id'),
                                        ('entity_id', 'pc_entities', 'entity_id'),
                                        ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                                        ...
                                        ('research_claim_id',
                                         'pc_research_claims',
                                         'research_claim_id')],
 'pc_security_operations': [('lead_entity_id', 'pc_entities', 'entity_id'),
                            ('source_id', 'pc_sources', 'source_id'),
                            ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_shipbuilding_production_tasks': [('shipbuilding_order_id',
                                       'pc_shipbuilding_orders',
                                       'shipbuilding_order_id'),
                                      ('shipbuilding_order_unit_id',
                                       'pc_shipbuilding_order_units',
                                      ...
                                      ('source_id', 'pc_sources', 'source_id'),
                                      ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_shipyard_capacity_history': [('shipyard_asset_id', 'pc_assets', 'asset_id'),
                                  ('operator_entity_id', 'pc_entities', 'entity_id'),
                                  ('source_id', 'pc_sources', 'source_id'),
                                  ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_supply_chain_dependencies': [('dependent_entity_id', 'pc_entities', 'entity_id'),
                                  ('dependent_asset_id', 'pc_assets', 'asset_id'),
                                  ('dependent_corridor_key', 'pc_trade_corridors', 'corridor_key'),
                                  ('provider_entity_id', 'pc_entities', 'entity_id'),
                                  ('provider_asset_id', 'pc_assets', 'asset_id'),
                                  ('provider_corridor_key', 'pc_trade_corridors', 'corridor_key'),
                                  ('source_id', 'pc_sources', 'source_id'),
                                  ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_trucking_fleet_assignments': [('entity_id', 'pc_entities', 'entity_id'),
                                   ('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                                   ('depot_asset_id', 'pc_assets', 'asset_id'),
                                   ('transport_service_id',
                                    'pc_transport_services',
                                    'transport_service_id'),
                                   ('source_id', 'pc_sources', 'source_id'),
                                   ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_vessel_deployments': [('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                           ('operator_entity_id', 'pc_entities', 'entity_id'),
                           ('transport_service_id', 'pc_transport_services', 'transport_service_id'),
                           ('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                           ('source_id', 'pc_sources', 'source_id'),
 ...
 'pc_vessel_identity_history': [('mobile_asset_id', 'pc_mobile_assets', 'mobile_asset_id'),
                                ('source_id', 'pc_sources', 'source_id'),
                                ('research_claim_id', 'pc_research_claims', 'research_claim_id')],
 'pc_weather_observations': [('corridor_key', 'pc_trade_corridors', 'corridor_key'),
                             ('event_id', 'pc_events', 'event_id'),
                             ('source_id', 'pc_sources', 'source_id'),
                             ('source_record_uuid', 'pc_source_records', 'source_record_uuid')]}
PHASE13_PRIORITY = {'pc_air_service_movements': 48,
 'pc_airspace_disruptions': 43,
 'pc_canonical_identity_decisions': 32,
 'pc_claim_evidence': 32,
 'pc_company_asset_roles': 33,
 'pc_company_corridor_roles': 48,
 'pc_company_event_effects': 43,
 'pc_company_milestones': 45,
 'pc_company_offices': 23,
 'pc_company_people_roles': 26,
 'pc_company_portfolio_positions': 45,
 'pc_corridor_exposures': 43,
 'pc_corridor_mode_connections': 48,
 'pc_corridor_variants': 32,
 'pc_cruise_voyage_calls': 43,
 'pc_cruise_voyages': 39,
 'pc_defence_organisations': 32,
 'pc_defence_programme_milestones': 59,
 'pc_defence_programme_participants': 52,
 'pc_defence_programmes': 49,
 'pc_event_updates': 43,
 'pc_freight_rate_assessments': 59,
 'pc_inland_waterway_status': 43,
 'pc_logistics_event_impacts': 51,
 'pc_logistics_service_agreements': 48,
 'pc_logistics_shipment_legs': 48,
 'pc_logistics_shipments': 32,
 'pc_market_corridor_links': 59,
 'pc_monitoring_alerts': 46,
 'pc_monitoring_observations': 43,
 'pc_monitoring_profiles': 26,
 'pc_monitoring_rules': 29,
 'pc_passenger_incident_impacts': 43,
 'pc_people': 8,
 'pc_port_service_disruptions': 48,
 'pc_rail_network_disruptions': 43,
 'pc_rail_rolling_stock_units': 33,
 'pc_rail_service_runs': 48,
 'pc_research_claims': 29,
 'pc_research_hypotheses': 26,
 'pc_research_opportunities': 32,
 'pc_research_projects': 23,
 'pc_research_questions': 26,
 'pc_road_border_observations': 43,
 'pc_road_disruptions': 43,
 'pc_road_network_links': 32,
 'pc_screening_cases': 23,
 'pc_screening_matches': 26,
 'pc_security_attribution_claims': 43,
 'pc_security_operation_participants': 35,
 'pc_security_operations': 32,
 'pc_shipbuilding_production_tasks': 59,
 'pc_shipyard_capacity_history': 32,
 'pc_supply_chain_dependencies': 32,
 'pc_trucking_fleet_assignments': 48,
 'pc_vessel_deployments': 48,
 'pc_vessel_identity_history': 33,
 'pc_weather_observations': 43}

# Do not assume a proposed migration script has been run.
# Activation of the 13A/B and 13C-F tables requires live registration.
# Canonical writes are separately gated by opt-in; staging must not
# create supposed canonical parent entities, vessels or infrastructure.
PHASE13_APPLY_ENABLED = os.getenv("PC_PHASE13_APPLY_ENABLED", "false").lower() == "true"

AI_ALLOWED_TABLES.update(PHASE13_TABLES)
UUID_ID_FIELDS.update({'pc_air_service_movements': 'air_service_movement_id',
 'pc_airspace_disruptions': 'airspace_disruption_id',
 'pc_canonical_identity_decisions': 'identity_decision_id',
 'pc_claim_evidence': 'claim_evidence_id',
 'pc_company_asset_roles': 'company_asset_role_id',
 ...
 'pc_vessel_identity_history': 'vessel_identity_history_id',
 'pc_weather_observations': 'weather_observation_id'})
REQUIRED_BY_TABLE.update({t: list(v) for t,v in PHASE13_REQUIRED.items()})
FK_RULES.update({t: list(v) for t,v in PHASE13_FK.items()})
APPLY_CONFLICT_KEYS.update(PHASE13_PK)

def _phase13_registered(sb, table):
    """Fail closed if a Phase 13 entity type/column registry is inaccessible."""
    if table not in PHASE13_TABLES: return True
    try:
        et=(sb.table("pc_meta_entity_types")
            .select("entity_type,primary_key_column,active,allow_insert")
            .eq("table_name",table).eq("active",True).limit(2)
            .execute().data or [])
        if len(et)!=1 or et[0].get("primary_key_column")!=PHASE13_PK[table]:
            return False
        cols=(sb.table("pc_meta_columns")
            .select("column_name")
            .eq("table_name",table).limit(200).execute().data or [])
        actual={x.get("column_name") for x in cols}
        return set(PHASE13_COLUMNS[table]) <= actual
    except Exception:
        return False

def _phase13_prepare_stage_payload(sb, table, payload, natural_key):
    """Stage without any canonical writes or automatic parent creation.

    Deterministic UUID generation is based only on an explicit stable natural
    key. A shared PK on a canonical parent (e.g. defence organisations) must
    be supplied by the analyst after identity matching.
    """
    if not _phase13_registered(sb,table):
        raise ValueError(f"{table}: Phase 13 physical table/registry not available; verify migration first")
    p=dict(payload)
    pk=PHASE13_PK[table]
    if table in UUID_ID_FIELDS and pk==UUID_ID_FIELDS[table]:
        raw=p.get(pk)
        if raw not in (None,""):
            try: p[pk]=str(uuid.UUID(str(raw)))
            except Exception: raise ValueError(f"{table}.{pk}: invalid UUID; do not silently replace it")
        elif natural_key and not str(natural_key).startswith(table+":"):
            p[pk]=str(uuid.uuid5(uuid.NAMESPACE_URL,
                f"power-corridors|phase13|{table}|{natural_key}"))
        else:
            raise ValueError(f"{table}: explicit stable natural_key or UUID required")
    elif p.get(pk) in (None,""):
        raise ValueError(f"{table}.{pk}: resolve existing canonical parent first")
    # Only physical columns are sent to PostgreSQL. Unmapped source content
    # remains in the staging metadata if the table supports that column.
    physical=set(PHASE13_COLUMNS[table])
    overflow={k:v for k,v in p.items() if k not in physical and v not in (None,"")}
    p={k:v for k,v in p.items() if k in physical and v not in (None,"")}
    if overflow and "metadata" in physical:
        meta=p.get("metadata") if isinstance(p.get("metadata"),dict) else {}
        meta=dict(meta)
        meta.setdefault("loader_reference_context",{}).update(_jsonable(overflow))
        p["metadata"]=meta
    elif overflow:
        raise ValueError(f"{table}: unknown columns {sorted(overflow)}; retain in staged source, not canonical row")
    return p

def _phase13_schema_valid(table, payload):
    if table not in PHASE13_TABLES or not isinstance(payload,dict):
        return False,["invalid Phase 13 target/payload"]
    missing=[c for c in PHASE13_REQUIRED[table]
             if payload.get(c) in (None,"")]
    # Phase 13 UUID PK has a database default; loader still requires the ID
    # for deterministic staging and review. Shared parent keys are compulsory.
    pk=PHASE13_PK[table]
    if payload.get(pk) in (None,"") and pk not in missing: missing.append(pk)
    for operator,fields in PHASE13_CARDINALITY.get(table,[]):
        count=sum(payload.get(f) not in (None,"") for f in fields)
        if (operator=='=' and count!=1) or (operator=='>=' and count<1):
            missing.append(f"Expected {operator} 1 of {', '.join(fields)}; found {count}")
    return not missing,missing

def _phase13_fk_valid(sb,table,payload):
    errors=[]
    for field,ref_table,ref_col in PHASE13_FK[table]:
        value=payload.get(field)
        if value in (None,""): continue
        try:
            rows=(sb.table(ref_table).select(ref_col).eq(ref_col,value).limit(1).execute().data or [])
            if not rows: errors.append(f"{field} -> {ref_table}.{ref_col}: missing")
        except Exception as exc:
            errors.append(f"{field}: database check unavailable ({type(exc).__name__})")
    return not errors,errors

def _phase13_require_apply(sb,table,payload,staged_row=None):
    if not PHASE13_APPLY_ENABLED:
        raise ValueError("Phase 13 canonical apply disabled; set PC_PHASE13_APPLY_ENABLED=true AFTER integration tests")
    if not _phase13_registered(sb,table):
        raise ValueError(f"{table}: live model registration incomplete or unavailable")
    ok,missing=_phase13_schema_valid(table,payload)
    if not ok: raise ValueError(f"{table}: required fields missing: {missing}")
    fkok,errors=_phase13_fk_valid(sb,table,payload)
    if not fkok: raise ValueError(f"{table}: FK check failed: {errors}")
    if not any(payload.get(field) not in (None,"") for field in ("source_id","research_claim_id","document_id","source_record_uuid")):
        if _source_count(payload)<1:
            raise ValueError(f"{table}: source_id, research claim, or source URL required")
    if table=="pc_defence_organisations":
        # No invented organization IDs or new parent side effects.
        entity_id=payload.get("entity_id")
        hit=(sb.table("pc_entities").select("entity_id")
             .eq("entity_id",entity_id).limit(1).execute().data or [])
        if not hit: raise ValueError("Defence organization must reference a resolved pc_entities identity")
    return True

def _phase13_readiness(sb):
    rows=[]
    for phase,tables in PHASE13_TABLES_BY_PHASE.items():
        for table in tables:
            rows.append({"phase":"13"+phase,"table":table,
                         "registered":_phase13_registered(sb,table),
                         "write_enabled":PHASE13_APPLY_ENABLED})
    return rows


PHASE13_CARDINALITY = {'pc_claim_evidence': [('>=', ('source_id', 'source_record_uuid', 'document_id', 'source_url'))],
 'pc_company_asset_roles': [('=', ('asset_id', 'mobile_asset_id'))],
 'pc_company_portfolio_positions': [('=', ('investee_entity_id', 'investee_asset_id'))],
 'pc_inland_waterway_status': [('>=', ('corridor_key', 'waterway_asset_id'))],
 'pc_logistics_event_impacts': [('>=',
                                 ('logistics_shipment_id',
                                  'shipment_leg_id',
                                  'transport_service_id',
                                  'corridor_key'))],
 'pc_passenger_incident_impacts': [('>=', ('mobile_asset_id', 'cruise_voyage_id', 'ferry_route_id'))],
 'pc_rail_network_disruptions': [('>=', ('rail_network_id', 'rail_link_id', 'node_asset_id'))],
 'pc_road_disruptions': [('>=', ('road_corridor_id', 'road_network_link_id', 'border_asset_id'))],
 'pc_security_attribution_claims': [('>=', ('claimed_actor_entity_id', 'claimed_actor_label'))],
 'pc_security_operation_participants': [('>=', ('entity_id', 'mobile_asset_id'))],
 'pc_shipbuilding_production_tasks': [('>=',
                                       ('shipbuilding_order_id',
                                        'shipbuilding_order_unit_id',
                                        'defence_programme_id'))],
...
def _prepare_universal_records(result,input_url,publisher=None,title=None):
    ...
    for rec in result.get("records") or []:
        ...
        if table in PHASE13_TABLES:
            nk=str(rec.get("natural_key") or payload.get(PHASE13_PK[table]) or "")
        else:
...
def _simple_direct_write_records(sb, result):
    ...
    all_clean=[r for r in records if isinstance(r,dict)]
    # Mixed old/new loads process legacy domains but block Phase13 direct writes.
    blocked=[r for r in all_clean if r.get("target_table") in PHASE13_TABLES]
    clean=[r for r in all_clean if r.get("target_table") not in PHASE13_TABLES]
    ...
    applied=0; failures=[{
        "table":r.get("target_table"),"record":r.get("natural_key"),
        "error":"Phase 13 direct write disabled; use controlled staging and analyst review",
        "payload_keys":sorted(list((r.get("payload") or {}).keys())) if isinstance(r.get("payload"),dict) else [],
    } for r in blocked]; by_table={}; attempted_by_table={}
    for _r in blocked:
        _table=str(_r.get("target_table") or "unknown")
        attempted_by_table[_table]=attempted_by_table.get(_table,0)+1
    ...
    return {
        "attempted":len(all_clean),"applied":applied,"blocked":len(failures),"failed":len(failures),
        "failures":failures,"by_table":by_table,"attempted_by_table":attempted_by_table,
        "failed_by_table":failed_by_table,
        "canonical_complete":len(completeness_issues)==0 and not blocked,
...
def _research_workbook_kind(sections,filename=""):
    ...
    # are loaded instead of falling through to generic reconciliation.
    portsmouth_required={"port_system","berths_infrastructure","ferry_routes","ferry_vessels","companies_entities"}
    portsmouth_domain={"royal_navy_fleet","naval_infrastructure","cruise_schedule","cargo_logistics","projects_investment"}
    if (portsmouth_required.issubset(names) and len(portsmouth_domain.intersection(names))>=3) or "portsmouth" in fname:
        return "portsmouth_system"
    ...
    # V5.3 generic fallback: any workbook with at least one non-empty data sheet is
    # treated as a schema-driven research package. Dedicated handlers above remain
    # useful hints, but are no longer required for a workbook to load.
    if names:
        return "generic_schema_driven"
    return None
...
def _portsmouth_system_workbook_records(sections):
    """Normalize the Portsmouth port-system workbook across the full logistics cycle.

    The workbook spans commercial ports, berths, ferry routes/vessels, cargo services,
    companies, Royal Navy assets, naval infrastructure, cruise calls, economic metrics,
    projects, news and local maritime-service companies.  Every supported row is loaded
    directly; residual research/data-quality rows are retained as observations.
    """
    kind="portsmouth_system"
    records=[]

    def rows(sheet):
        return _rw_rows(sections.get(sheet,pd.DataFrame()))

    port_rows=rows("Port_System")
    port_by_id={str(r.get("port_asset_id")):r for r in port_rows if r.get("port_asset_id")}
    entity_rows=rows("Companies_Entities")
    entity_by_id={str(r.get("entity_id")):r for r in entity_rows if r.get("entity_id")}

    def port_name(local_id):
        r=port_by_id.get(str(local_id or ""),{})
    ...
    def split_names(value):
        if value in (None,""):
            return []
        s=str(value).strip()
        # Keep slashes inside ordinary names where possible, but split common multi-party delimiters.
    ...
    for row in rows("Sources"):
        ...
        records.append(_rw_record("pc_sources",f"source:{url}",{
            ...
            "publisher":row.get("source_name") or host,
            "source_type":row.get("source_type") or "web",
            "url":url,"ingestion_method":"portsmouth_system_workbook","active":True,
            "metadata":_rw_metadata(kind,"Sources",row,{"used_for":row.get("used_for")}),
        },1.0))
    ...
    for row in entity_rows:
        ...
        records.append(_rw_record("pc_entities",f"entity:{name}",{
            "name":name,"entity_type":row.get("entity_type") or "company",
            "country":"United Kingdom","hq_country":"United Kingdom",
            "source_url":url,"record_status":"verified",
            "metadata":_rw_metadata(kind,"Companies_Entities",row,{
                "workbook_entity_id":row.get("entity_id"),
                "role_in_portsmouth":row.get("role_in_portsmouth"),
                "location_or_address":row.get("location_or_address"),
                "current_metric_or_note":row.get("current_metric_or_note"),
            }),
        },1.0))
        parent=row.get("ownership_parent")
        if parent:
    ...
    # Local maritime directory companies -------------------------------------
    for row in rows("Maritime_Directory"):
        name=row.get("company")
        if not name: continue
        records.append(_rw_record("pc_entities",f"entity:portsmouth-directory:{name}",{
            "name":name,"entity_type":"maritime services company","country":"United Kingdom",
            "source_url":row.get("source_url"),"record_status":"verified",
            "metadata":_rw_metadata(kind,"Maritime_Directory",row,{
                "service_category":row.get("category"),"location":row.get("location"),"role":row.get("role")
            }),
        },1.0))
    ...
    for row in port_rows:
        ...
        atype=row.get("asset_type") or "port"
        records.append(_rw_record("pc_assets",f"asset:portsmouth:{name}",{
            "name":name,"asset_type":atype,"country":"United Kingdom","region_city":"Portsmouth",
            "status":row.get("status"),"operator_entity_name":owner,"owner_entity_name":owner,
            "source_url":row.get("source_url"),"record_status":"verified",
            "metadata":_rw_metadata(kind,"Port_System",row,{
                "workbook_port_asset_id":row.get("port_asset_id"),"address_or_area":row.get("address_or_area"),
                "primary_role":row.get("primary_role"),"scale_or_metric":row.get("scale_or_metric")
            }),
        },1.0))
        if "port" in str(atype).casefold() or "harbour" in str(atype).casefold() or "naval base" in str(atype).casefold():
            records.append(_rw_record("pc_port_capabilities",f"port-capability:{name}",{
                "port_asset_name":name,"source_url":row.get("source_url"),
                "metadata":_rw_metadata(kind,"Port_System",row,{
                    "primary_role":row.get("primary_role"),"scale_or_metric":row.get("scale_or_metric")
                }),
            },1.0))
    ...
    for row in rows("Berths_Infrastructure"):
        name=row.get("name") or row.get("berth_id")
        if not name: continue
        parent=port_name(row.get("port_asset_id"))
        url=row.get("source_url")
        records.append(_rw_record("pc_assets",f"asset:berth:{name}:{parent}",{
            "name":name,"asset_type":"berth","subtype":row.get("berth_type"),"country":"United Kingdom",
            "region_city":"Portsmouth","status":row.get("status"),"source_url":url,"record_status":"verified",
            "metadata":_rw_metadata(kind,"Berths_Infrastructure",row,{"parent_port":parent,"workbook_berth_id":row.get("berth_id")}),
        },1.0))
        records.append(_rw_record("pc_berth_details",f"berth-detail:{name}:{parent}",{
            "berth_name":name,"parent_port_name":parent,"berth_type":row.get("berth_type"),
            "length_m":_rw_number(row.get("length_m")),"maintained_depth_m_cd":_rw_number(row.get("maintained_depth_m_cd")),
            "max_vessel_length_m":_rw_number(row.get("max_vessel_length_m")),"max_beam_m":_rw_number(row.get("max_beam_m")),
            "shore_power":row.get("shore_power"),"status":row.get("status"),"source_url":url,
            "metadata":_rw_metadata(kind,"Berths_Infrastructure",row,{"known_use_or_feature":row.get("known_use_or_feature")}),
        },1.0))

    # Ferry routes -> services + ordered endpoints ---------------------------
    for row in rows("Ferry_Routes"):
        route_id=row.get("route_id")
        origin=port_name(row.get("origin_port_asset"))
        dest=row.get("destination")
        operator=row.get("operator")
        if not origin or not dest: continue
        sname=f"{operator}: {origin}–{dest}" if operator else f"{origin}–{dest}"
        sid=_simple_hash_id("SERVICE_AI",route_id or sname)
        records.append(_rw_record("pc_transport_services",f"service:{route_id or sname}",{
            "transport_service_id":sid,"service_name":sname,"mode":"sea","service_type":row.get("service_type") or "ferry service",
            "primary_operator_entity_name":operator,"status":row.get("status") or "active","trade_lane":f"{origin}–{dest}",
            "source_url":row.get("source_url"),
            "metadata":_rw_metadata(kind,"Ferry_Routes",row,{"crossing_time":row.get("crossing_time"),"frequency":row.get("frequency"),"current_vessels_or_notes":row.get("current_vessels_or_notes")}),
        },1.0))
        for seq,pname in ((1,origin),(2,dest)):
            records.append(_rw_record("pc_transport_service_stops",f"service-stop:{sid}:main:{seq}:{pname}",{
                "transport_service_id":sid,"direction":"main","sequence_no":seq,"asset_name":pname,"asset_type":"port",
                "call_type":"scheduled","source_url":row.get("source_url"),"metadata":_rw_metadata(kind,"Ferry_Routes",row),
            },1.0))

    # Ferry vessels -----------------------------------------------------------
    for row in rows("Ferry_Vessels"):
        name=row.get("vessel_name")
        if not name: continue
        records.append(_rw_record("pc_mobile_assets",f"mobile:ferry:{name}",{
            "name":name,"asset_type":"vessel","subtype":row.get("vessel_type"),"operator_entity_name":row.get("operator"),
            "status":row.get("status"),"source_url":row.get("source_url"),
            "metadata":_rw_metadata(kind,"Ferry_Vessels",row,{
                "workbook_vessel_id":row.get("vessel_id"),"route_or_port_link":row.get("route_or_port_link"),
                "length_m":row.get("length_m"),"passenger_capacity":row.get("passenger_capacity"),
                "vehicle_or_lane_capacity":row.get("vehicle_or_lane_capacity"),"propulsion_or_feature":row.get("propulsion_or_feature")
            }),
        },1.0))

    # Cargo services / logistics observations --------------------------------
    for row in rows("Cargo_Logistics"):
        label=row.get("service_or_cargo") or row.get("cargo_record_id")
        if not label: continue
        terminal=row.get("terminal"); operator=row.get("operator_or_line"); lane=row.get("route_or_market")
        # Rows with an operator/line and route/market are operational transport/logistics services.
        if operator and lane:
            sname=f"{operator}: {label}"
            records.append(_rw_record("pc_transport_services",f"cargo-service:{row.get('cargo_record_id') or sname}",{
                "transport_service_id":_simple_hash_id("SERVICE_AI","portsmouth-cargo",row.get("cargo_record_id") or sname),
                "service_name":sname,"mode":"sea","service_type":"freight service","primary_operator_entity_name":operator,
                "status":row.get("status"),"trade_lane":lane,"source_url":row.get("source_url"),
                "metadata":_rw_metadata(kind,"Cargo_Logistics",row,{"terminal":terminal,"capacity_or_metric":row.get("capacity_or_metric"),"infrastructure":row.get("infrastructure")}),
            },1.0))
        records.append(_rw_record("pc_observations",f"cargo-observation:{row.get('cargo_record_id') or label}",{
            "observation_id":_simple_hash_id("OBS_AI","portsmouth-cargo",row.get("cargo_record_id") or label),
            "source_type":"cargo_logistics","source_url":row.get("source_url"),"confidence":"high",
            "review_status":"approved","record_status":"verified","raw_value":_jsonable(row),
            "metadata":_rw_metadata(kind,"Cargo_Logistics",row),
        },1.0))

    # Royal Navy fleet --------------------------------------------------------
    for row in rows("Royal_Navy_Fleet"):
        name=row.get("name_or_class")
        if not name: continue
        records.append(_rw_record("pc_mobile_assets",f"mobile:royal-navy:{name}",{
            "name":name,"asset_type":"naval vessel" if "class" not in str(row.get("asset_type") or "").casefold() else "naval class",
            "subtype":row.get("asset_type"),"operator_entity_name":"Royal Navy","status":"active","source_url":row.get("source_url"),
            "metadata":_rw_metadata(kind,"Royal_Navy_Fleet",row,{
                "workbook_naval_asset_id":row.get("naval_asset_id"),"homeport_status":row.get("homeport_status"),
                "key_spec_or_count":row.get("key_spec_or_count"),"current_2026_context":row.get("current_2026_context"),
                "support_entities":row.get("support_entities")
            }),
        },1.0))
        # Explicit base relationship where the row says Portsmouth-based/homeport.
        if "portsmouth" in str(row.get("homeport_status") or "").casefold():
            records.append(_rw_record("pc_relationships",f"relationship:{name}:homeported:HMNB Portsmouth",{
                "source_type":"mobile_asset","source_name":name,"source_subtype":row.get("asset_type"),
                "relationship_type":"homeported at","target_type":"asset","target_name":"HMNB Portsmouth",
                "target_asset_type":"naval base","confidence":"high","source_url":row.get("source_url"),
                "metadata":_rw_metadata(kind,"Royal_Navy_Fleet",row),
            },1.0))

    # Naval infrastructure + investment projects -----------------------------
    for sheet in ("Naval_Infrastructure","Projects_Investment"):
        for row in rows(sheet):
            pname=row.get("asset_or_program") or row.get("project") or row.get("project_id")
            if not pname: continue
            url=row.get("source_url")
            leads=row.get("lead_entity") or row.get("lead_entities")
            lead_first=split_names(leads)[0] if split_names(leads) else (str(leads).split(" / ")[0].strip() if leads else None)
            status=row.get("status")
            records.append(_rw_record("pc_assets",f"project-asset:{sheet}:{pname}",{
                "name":pname,"asset_type":"project","country":"United Kingdom","region_city":"Portsmouth",
                "status":status,"operator_entity_name":lead_first,"source_url":url,"record_status":"verified",
                "metadata":_rw_metadata(kind,sheet,row,{"lead_entities":leads}),
            },1.0))
            records.append(_rw_record("pc_project_details",f"project-detail:{sheet}:{pname}",{
                "asset_name":pname,"project_type":"naval infrastructure" if sheet=="Naval_Infrastructure" else "port/logistics investment",
                "project_stage":status,"sponsor_entity_name":lead_first,"source_url":url,
                "metadata":_rw_metadata(kind,sheet,row),
            },1.0))

    # Cruise schedule -> mobile assets + scheduled port calls -----------------
    default_cruise_port="Portsmouth International Port"
    for row in rows("Cruise_Schedule"):
        ship=row.get("ship")
        if not ship: continue
        url=row.get("source_url")
        records.append(_rw_record("pc_mobile_assets",f"mobile:cruise:{ship}",{
            "name":ship,"asset_type":"vessel","subtype":"cruise ship","operator_entity_name":row.get("cruise_line"),
            "status":"active","source_url":url,"metadata":_rw_metadata(kind,"Cruise_Schedule",row),
        },1.0))
        port=port_name(row.get("port_asset_id")) or default_cruise_port
        call_key=f"{ship}:{row.get('date')}:{port}:{row.get('eta')}"
        records.append(_rw_record("pc_port_calls",f"port-call:{call_key}",{
            "vessel_name":ship,"port_name":port,"call_type":row.get("call_type") or "scheduled cruise call",
            "arrival_time":row.get("eta"),"departure_time":row.get("etd"),"call_date":_rw_date(row.get("date")),
            "status":"scheduled","source_url":url,
            "metadata":_rw_metadata(kind,"Cruise_Schedule",row,{"provisional":True,"cruise_line":row.get("cruise_line")}),
        },1.0))

    # Economic metrics --------------------------------------------------------
    for row in rows("Economic_Stats"):
        metric=row.get("metric")
        if not metric: continue
        records.append(_rw_record("pc_observations",f"economic:{metric}:{row.get('period')}:{row.get('scope')}",{
            "observation_id":_simple_hash_id("OBS_AI","portsmouth-economic",metric,row.get("period"),row.get("scope")),
            "source_type":"economic_metric","source_url":row.get("source_url"),"confidence":"high",
            "review_status":"approved","record_status":"verified","raw_value":_jsonable(row),
            "metadata":_rw_metadata(kind,"Economic_Stats",row),
        },1.0))

    # News/events -------------------------------------------------------------
    for row in rows("News_12M"):
        title=row.get("headline")
        if not title: continue
        eid=_simple_hash_id("EVENT_RWB","portsmouth",row.get("date"),title)
        url=row.get("source_url")
        records.append(_rw_record("pc_events",f"event:{eid}",{
            "event_id":eid,"start_date":_rw_date(row.get("date")),"event_domain":"logistics",
            "event_nature":row.get("domain"),"event_type":row.get("domain") or "news/development",
            "event_category":row.get("domain"),"title":title,"description":row.get("summary"),
            "trade_relevance":3,"intelligence_relevance":3,"trade_visible":True,"intelligence_visible":True,
            "status":"recorded","source_url":url,"metadata":_rw_metadata(kind,"News_12M",row),
        },1.0))
        for en in split_names(row.get("entities")):
            # Ship names are still safe as entity links here; the event retains full source row.
            records.append(_rw_record("pc_event_links",f"event-link:{eid}:entity:{en}",{
                "event_id":eid,"linked_type":"entity","linked_name":en,"relationship":"mentioned/affected party",
                "confidence":"high","source_url":url,"metadata":_rw_metadata(kind,"News_12M",row),
            },1.0))

    # Data-quality notes are valuable source-scoped observations, not load blockers.
    for row in rows("Data_Quality"):
        issue=row.get("issue")
        if not issue: continue
        records.append(_rw_record("pc_observations",f"data-quality:{issue}",{
            "observation_id":_simple_hash_id("OBS_AI","portsmouth-dq",issue),"source_type":"data_quality_note",
            "source_url":row.get("source_url"),"confidence":"high","review_status":"approved","record_status":"verified",
            "raw_value":_jsonable(row),"metadata":_rw_metadata(kind,"Data_Quality",row),
        },1.0))

    # Preserve summary rows as observations so no research content disappears.
    for idx,row in enumerate(rows("Summary"),start=1):
        records.append(_rw_record("pc_observations",f"summary:{idx}:{row}",{
            "observation_id":_simple_hash_id("OBS_AI","portsmouth-summary",idx,str(row)),"source_type":"workbook_summary",
            "confidence":"high","review_status":"approved","record_status":"verified","raw_value":_jsonable(row),
            "metadata":_rw_metadata(kind,"Summary",row),
        },1.0))
...
# ---------------------------------------------------------------------------
# V5.3 generic schema-driven workbook normalizer
# ---------------------------------------------------------------------------

def _generic_first(row, *keys):
    for k in keys:
        v=row.get(k)
        if v not in (None, ""):
            return v
...
def _generic_cols(row):
    return {_norm_field(k) for k,v in (row or {}).items() if v not in (None,"")}
...
def _generic_is_sanctions_sheet(sheet_norm, rows):
    if any(x in sheet_norm for x in ("ofac","sanction","sdn","non_sdn","designation")):
        return True
    sample=rows[:5]
    keys=set()
    for r in sample: keys |= _generic_cols(r)
...
def _generic_schema_driven_workbook_records(sections):
    ...
    for sn,(sheet,rows) in sheet_rows.items():
        ...
        for i,row in enumerate(rows,1):
            ...
            cols=_generic_cols(row)
            md=_rw_metadata(kind,sheet,row,{"generic_classifier":True})
            ...
            if "service_name" in cols or ("route_name" in cols and cols.intersection({"origin","destination","sequence","port_name","station_name","stop_name"})):
                ...
                if sname:
                    sid=_simple_hash_id("SERVICE_AI",sname,_generic_first(row,"operator","company","carrier"))
            ...
            # --- mobile assets ---------------------------------------------
            if _generic_is_mobile(row,sn):
                name=_generic_first(row,"vessel_name","ship_name","aircraft_name","name","asset_name","model_or_type")
                if name:
...
def stage_ai_result(sb, job_id, result):
    ...
    records=sorted(
        ...
        key=lambda r: PHASE13_PRIORITY.get(str(r.get("target_table") or ""),
                                           priority.get(str(r.get("target_table") or ""),60))
    )
    ...
    for rec in records:
        ...
        if table in PHASE13_TABLES:
            proposed_key=rec.get("natural_key") or payload.get(PHASE13_PK[table])
            if not proposed_key:
                rejected+=1
                errors.append({"table":table,"natural_key":None,
                               "error":"Phase 13 requires stable natural_key or explicit primary key"})
                continue
            natural_key=str(proposed_key)
        else:
            natural_key=_record_key(payload,rec.get("natural_key") or "")
        try:
            if table in PHASE13_TABLES:
                payload=_phase13_prepare_stage_payload(sb,table,payload,natural_key)
            else:
        ...
        if table in PHASE13_TABLES:
            schema_ok,missing=_phase13_schema_valid(table,payload)
            # Parent rows included in same job may not yet exist in DB.
            # They remain REVIEW, never misrepresented as FK-verified READY.
            fk_ok,fk_issues=_phase13_fk_valid(sb,table,payload)
            phase13_ready=schema_ok and fk_ok and bool(logical)
            if not schema_ok:
                errors.append({"table":table,"natural_key":natural_key,
                               "error":"Missing required fields: "+", ".join(missing)})
            if not fk_ok:
                errors.append({"table":table,"natural_key":natural_key,
                               "error":"FK dependencies need review: "+"; ".join(fk_issues)})
        else:
            phase13_ready=True
        staged_row={
            "ingestion_job_id":job_id,
            ...
            "validation_status":"pending",
            "review_status":"pending",
            "resolution_status":"READY" if phase13_ready else "UNRESOLVED",
            "resolution_method":("phase13_registry_and_fk_checked" if table in PHASE13_TABLES and phase13_ready
                                 else "phase13_manual_dependency_review" if table in PHASE13_TABLES
                                 else "simple_key_first" if key_ready else "simple_direct_insert"),
        }
...
def apply_staged_record(sb, row, edited_payload=None):
    ...
    payload=_jsonable(payload)
    if table in PHASE13_TABLES:
        # No direct writes, implicit parent creation, or unverified FK applies.
        _phase13_require_apply(sb,table,payload,row)
...
10. Verify client views""")
    ...
    if not sb:
        ...
        st.markdown("#### Phase 13 multimodal readiness")
        st.caption("Offline code covers 58 Phase 13 specialist tables. Live registration must pass before staging; canonical apply is opt-in and disabled by default.")
        if st.checkbox("Check Phase 13 registration (58 tables)",value=False,key="pc_phase13_readiness"):
            with st.spinner("Checking live registry for Phase 13 domains..."):
                _p13_rows=_phase13_readiness(sb)
            _ok=sum(bool(r["registered"]) for r in _p13_rows)
            st.metric("Phase 13 registered tables",f"{_ok} / {len(_p13_rows)}")
            st.dataframe(pd.DataFrame(_p13_rows),use_container_width=True,hide_index=True)
        st.caption("Document Loader and Sanctions Bulk Load remain unchanged. No Phase 13 database writes occur during readiness checks.")
